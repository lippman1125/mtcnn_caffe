...
def GenerateData(ftxt, data_path, save_dir, size, argument=False):
    '''
    ...
    :param size: one of the net in the cascaded networks
    ...
    image_id = 0
    # landmark list
    f = open(join(save_dir, "landmark_%s.txt" %(size)),'w')
    ...
    for (imgPath, bbox, landmarkGt) in data:
        ...
        gt_box = np.array([bbox.left,bbox.top,bbox.right,bbox.bottom], dtype = np.float32)
        ...
        if argument:
            ...
            for i in range(len(F_imgs)):
                ...
                # write image
                cv2.imwrite(join(save_dir,"landmark/%d.jpg" %(image_id)), F_imgs[i])
                landmarks = map(str,list(F_landmarks[i]))
                # write image list
                f.write(join(save_dir,"landmark/%d" %(image_id))+" -2 "+" ".join(landmarks)+"\n")
                image_id = image_id + 1
...
if __name__ == '__main__':
    ...
    size = 24
    landmark_save_dir = '../{}net/{}'.format(size, size)
    img_dir = 'lfw'
    if not exists(os.path.join(landmark_save_dir, "landmark")):
        os.mkdir(os.path.join(landmark_save_dir, "landmark"))

    if not exists(img_dir):
        print("{} does not exist, exit".format(img_dir))
        exit()

    #the file contains the names of all the landmark training data
    train_txt = "trainImageList.txt"
    imgs,landmarks = GenerateData(train_txt, img_dir, landmark_save_dir, size, argument=True)
