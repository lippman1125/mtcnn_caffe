...
class MtcnnDetector(object):
    ...
    def detect_pnet(self, im):
        ...
        while min(current_height, current_width) > net_size:
            ...
            self.pnet_detector.blobs['data'].reshape(1, 3, current_height, current_width)
            self.pnet_detector.blobs['data'].data[...] = np.transpose(im_resized, (2,0,1))
            out_ = self.pnet_detector.forward()
            cls_cls_map = out_['prob1'][0]
            reg = out_['conv4-2'][0]
            # print(np.shape(cls_cls_map))
            # print(np.shape(reg))
            cls_cls_map = np.transpose(cls_cls_map, (1,2,0))
    ...
    def detect_rnet(self, im, dets):
        ...
        self.rnet_detector.blobs['data'].reshape(num_boxes, 3, 24, 24)
